...
pub fn books_list_view<'a>(layout: &Layout) -> Element<'static, Message> {
    let books_mapped = match layout.loading_books {
        true => (0..30)
            .map(|_| {
                //
                column![center("Loading").style(container::rounded_box),]
                    .width(200)
                    .height(300)
                    .into()
            })
        ...
        false => layout
            ...
            .map(|book| {
                ...
                .on_press(Message::SetCurrentBook(book.id.clone()));
    ...
    let content = container(
        scrollable(
            column![row(books_mapped).spacing(15).wrap(),]
                .spacing(40)
                .align_x(Center)
                .width(Fill),
        )
...
impl BooksList {
    ...
    pub async fn fetch_books(&self) -> Result<Vec<Book>, reqwest::Error> {
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            let books_with_images = join_all(books.into_iter().map(|book| {
                let book = book.clone();
                async move {
